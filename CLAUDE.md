...
종목 클릭 시 토스증권 Open API로 캐싱된 일봉 캔들 차트(이동평균선 포함)도 조회 가능.
...
python 뉴스분석.py  ← 장마감 후 실행
    ...
    ├── 토스증권 캔들 미리 조회 → MongoDB candles 컬렉션 캐싱
    ├── Naver 뉴스 API 검색
...
python 저장분석.py → MongoDB ai_analysis 컬렉션 저장
         ↓
웹앱: /api/getData, /api/getAnalysis, /api/tossQuote 가 MongoDB만 읽어서 화면 표시
```

> ⚠️ **토스증권 API는 IP 허용 목록 기반**이라 Vercel 서버리스 함수(유동 IP)에서 직접 호출하면
> 막힌다. 그래서 토스 API 호출은 고정 IP인 로컬 `뉴스분석.py`에서만 하고, Vercel은 MongoDB에
> 캐싱된 결과만 읽는다. 자세한 내용·이유는 [DATA_PIPELINE.md](DATA_PIPELINE.md) 참고.
...
- **종목 클릭 캔들차트가 일부 종목만 보임** → 의도된 동작. 그날 거래대금/등락률 상위 50에 든
  종목만 캐싱됨 ([DATA_PIPELINE.md](DATA_PIPELINE.md)).
