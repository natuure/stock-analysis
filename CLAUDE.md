...
(실시간 KIS 호출 아님).
분석 시 그날의 테마·종목을 14개 카테고리(반도체/조선/방산/AI로봇 등, **평소엔 이름·정의만
...
가장 부합하는 카테고리에 배치하면 되고 과거와 맞추지 않음. 기존 14개에 잘 안 맞는 종목은
...
- **14개 카테고리 목록에 새 업종이 추가돼 있지 않음(분명 반복 등장하는데도)** → 의도된
  동작. 평소엔 목록의 카테고리 이름·정의만 수정 가능하고 추가·삭제는 정식 절차 없이는 안
  함(2026-06-27 정정 — 한 번은 자유롭게 추가·삭제 가능하다고 했었음). 새 카테고리가
  필요해 보이는 종목은 일단 `기타` + `신규카테고리후보` 필드로 표시해두고, 도넛 차트의
  "기타" 범례를 펼치면 그 종목명이 빨간색으로 보임 — 그렇게 쌓인 후보를 보고 실제로 목록에
  추가할지는 사용자가 결정함 ([DATA_PIPELINE.md](DATA_PIPELINE.md)).
- **`저장분석.py` 실행 시 "카테고리 누락/오타가 있어 저장을 중단합니다" 에러가 뜸** → 의도된
  동작(2026-06-27 도입). 거래대금/등락률 종목은 카테고리가 필수라, 분석 JSON에서 하나라도
  빠지거나 14개 목록 밖 값(오타 등)이면, 또는 `신규카테고리후보`가 있는데 `카테고리`가
  `기타`가 아니면 어떤 종목이 문제인지 출력하고 MongoDB 저장 자체를 거부함. 14개 목록을
