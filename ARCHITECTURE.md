...
Python (로컬 실행, 고정 IP)
  ...
  차트분석.py   ← 뉴스분석.py 다음 실행, 등락률 상위 50 진입 종목을 30거래일 동안 추적해
                  MongoDB tracked_stocks 저장(웹앱 "차트분석" 탭용, 2026-06-30 도입)
```
...
│   │                             (선택 ?from=YYYY-MM-DD — 차트분석 탭의 "추적 종목 포함 이후 누적"
│   │                             조회용, 2026-06-30 추가, 아래 getTrackedStocks.js 참고)
...
│   ├── getTrackedStocks.js     # GET → tracked_stocks(차트분석.py가 채움)에서 status:'active'만 반환,
│   │                             종목이 리스트에 포함된 날부터 현재까지의 코스피/코스닥 누적
│   │                             변동률을 요청 시점에 stock_data로 계산해 같이 내려줌(2026-06-30)
...
│       ├── ChartAnalysis.jsx  # "차트분석" 탭 — tracked_stocks 표 + 행 클릭 시 "포함 이후 누적" 차트
│       │                       패널(StockChartPanel을 from/extraMaLines로 확장 재사용, 2026-06-30)
...
├── 차트분석.py              # 등락률 상위 50 진입 종목을 30거래일 동안 추적 → MongoDB tracked_stocks 저장
│                             (뉴스분석.py 다음 실행, 2026-06-30 도입)
...
| `tracked_stocks` | 등락률 상위 50 진입 종목의 30거래일 추적 상태(`차트분석.py`가 매번 `stock_data` 이력 전체를 리플레이해 채움, "차트분석" 탭용, 2026-06-30 도입) | `{ _id: "종목코드", name, market, status: "active"\|"removed", firstAddedDate, lastEnteredDate, referenceClose, ma: {asOf,currentPrice,ma50,ma150,ma200,aligned}, removedDate, removedReason }` — 자세한 규칙은 [DATA_PIPELINE.md](DATA_PIPELINE.md) 참고 |
...
| `/api/candles` | GET | `?symbol=&date=` → KIS Open API로 일봉 캔들 85개 실시간 조회. 실패 시에만 MongoDB `candles`(토스 캐시) 폴백. **`?from=YYYY-MM-DD` 추가 시** 고정 85개 대신 그 날짜부터 전체를 1회 조회(차트분석 탭 전용, 2026-06-30 — KIS 1회 호출 100개 한도 내에서만 정확, [DATA_PIPELINE.md](DATA_PIPELINE.md) 참고) |
...
| `/api/getTrackedStocks` | GET | `tracked_stocks`에서 `status:'active'`만 반환 + 종목이 리스트에 포함된 날부터 현재까지의 코스피/코스닥 누적 변동률을 `stock_data`로 요청 시점에 계산해 같이 내려줌("차트분석" 탭용, 2026-06-30) |
