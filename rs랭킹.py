...
결과:
  ...
  - **같은 컬렉션에 주차별 히스토리 문서도 함께 저장**(_id=weekKey, 예: "2026-W28",
    2026-07-11 추가 — "종목 분석" 탭의 "시장관심도"가 특정 종목의 최근 10주 RS Score
    추이를 보여주려면 그 주엔 백분위 90 미만이라 rsRank(top)에서 빠진 종목의 점수도
    필요하기 때문): {_id: weekKey, asOfDate, weekKey, scores: [{code, name, rsScore}, ...
    전종목(계산 성공한 전부, 카테고리 없음)], updatedAt}. api/getStockMarketInterest.js가
    이 문서들을 읽어 종목별 추이를 구성한다.
"""
...
def rs_ranking(db):
    ...
    if db is not None:
        ...
        # 주차별 전종목(백분위 90 미만 포함) 스냅샷 — "종목 분석" 탭의 "시장관심도"가 특정
        # 종목의 최근 10주 RS Score 추이를 보여주려면 그 종목이 어느 주엔 90 미만이라
        # rsRank(top)에서 빠졌더라도 그 주의 점수를 알아야 한다(2026-07-11 추가, 사용자
        # 요청). 카테고리는 이 용도에 필요 없어 넣지 않는다(카테고리·신규카테고리후보 조사는
        # top 리스트에만 필요). _id를 'latest'가 아니라 week_key로 둬서 같은 컬렉션 안에
        # 두 종류 문서(단일 최신 스냅샷 vs 주차별 히스토리)가 공존 — 'latest' 문서는 매번
        # 덮어써 최신 상태만 남고, week_key 문서는 주마다 쌓여 히스토리를 이룬다.
        history_scores = [{'code': s['code'], 'name': s['name'], 'rsScore': s['rsScore']} for s in scored]
        db[RS_RANKING_COLLECTION].update_one(
            {'_id': week_key},
            {'$set': {
                'asOfDate': as_of_date.isoformat(),
        ...
        print(f'MongoDB 저장 완료: {RS_RANKING_COLLECTION}/{week_key} (전종목 {len(history_scores)}개, RS 추이 조회용)')
    else:
