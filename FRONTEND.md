...
- **연도별 추이 차트**(2026-06-25 추가, 사용자 요청): 손익계산서·재무상태표 둘 다 단일 최근값
  ...
  주당순이익 3개 라인 차트는 표식마다 값을 숫자로 같이 보여줌**(`TrendChart`의 `showValues`
  prop, 2026-06-27 추가, 사용자 요청 — 자산 구성비·부채비율 등 다른 라인 차트는 적용 안 함,
  아래 "TrendChart.jsx" 절 참고). **분기 EPS를 보여주려고 종목분석.py의 `fetch_quarters()`에
...
9px)로 같이 그림 — 표식 라벨이 차트 위쪽 바깥으로 잘리지 않게 `showValues`일 때 위쪽
여백(`PAD_T`)을 10→22로 늘림. 기본값은 꺼짐이라 기존 차트(자산 구성비·부채비율·테마 카테고리
추이 등)는 영향 없고, `IncomeStatementView`의 영업이익률·ROE·주당순이익 3개 라인 차트만
명시적으로 켬.
