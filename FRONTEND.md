...
- **연도별 추이 차트**(2026-06-25 추가, 사용자 요청): 손익계산서·재무상태표 둘 다 단일 최근값
  ...
  손익계산서는 ① 매출액·영업이익·당기순이익 묶음 막대(억원) ② 영업이익률 막대(%, 1계열)
  ③ ROE 막대(%, 1계열) ④ 주당순이익 막대(원, 1계열) 4개, 재무상태표는 ① CAPEX·재고자산·
  매출채권·현금 비중 꺾은선(%, 4계열) ② 부채비율 꺾은선(%) ③ 자본금·자본잉여금·이익잉여금
  묶음 막대(억원, 3계열) ④ 선수금 막대(억원, 1계열) 4개를 그림. **②~④는 2026-06-27 ROE
  라인 추가 당시 영업이익률·ROE 한 차트 + 주당순이익 별도 차트였다가, 2026-06-28에
  ROE·주당순이익을 한 차트로 합치는 시도(단위가 달라 `TrendChart`에 보조 y축까지 추가)를
  거쳤으나 결국 사용자가 세 지표 모두 독립된 단일 계열 막대 차트로 되돌리는 쪽을 선택함** —
  `ROE = 당기순이익 ÷ 지배기업소유주지분`(분기는 지배기업소유주지분이 없어 자본총계로
  대체). `TrendChart`의 보조 y축(`axis:'right'`) 지원은 이 되돌림으로 더 쓰는 곳이
  없어져 같이 제거함(아래 "TrendChart.jsx" 절 참고). **재무상태표의 선수금도 같은 날
  자본금·자본잉여금·이익잉여금 묶음 막대에서 별도 차트로 분리됨** — 선수금이 회사별로
  나머지 세 계열보다 자릿수가 훨씬 작은 경우가 많아 같은 차트에 묶으면 막대가 거의 안
  보이는 문제 때문(사용자 제보). 값이 둘 이상인 계열(매출액·영업이익·당기순이익,
  CAPEX 등 비중, 자본금 등)은 캔들차트의 이동평균선 범례(`.candle-legend`)와 같은 패턴의
  `.trend-legend`로 색상-라벨을 표시. **영업이익률·ROE·주당순이익·부채비율·선수금 5개 단일
  계열 차트 + 매출액·영업이익·당기순이익 막대, 자본금·자본잉여금·이익잉여금 막대 차트는
  표식/막대마다 값을 숫자로 같이 보여줌**(`TrendChart`의 `showValues` prop — 막대는
  높이만으로는 정확한 값을 확인하기 어렵다는 사용자 피드백으로 라인 차트(2026-06-27 먼저
  추가)에 이어 막대도 지원 추가됨. 자본금·자본잉여금·이익잉여금·선수금 묶음은 처음엔
  손익계산서 막대 차트 요청 범위 밖이라 `showValues`를 안 켰었는데, 선수금을 분리하면서
  사용자가 둘 다 켜달라고 요청해 2026-06-28에 같이 켬, 아래 "TrendChart.jsx" 절 참고). **분기
...
- 주간(W##) 클릭 → 주간 요약 표시(코스피/코스닥) + **주간 거래대금/등락률 상위 50 표**
  ...
  `-`로 표시했었음). 반대로 `showCategory`는 켜서(2026-06-28 추가, 사용자 요청) "카테고리"
  열을 보여줌 — 일간 `vol`/`rate`에는 `카테고리` 필드가 전혀 없지만(일간은 카테고리를
  `ai_analysis` 쪽에서만 들고 프론트에서 종목명으로 매칭, 위 "거래대금·등락률 카테고리
  비중" 절 참고) 주간 `vol`/`rate`는 `주간분석.py`가 매칭한 `카테고리`를 항목에 직접
  박아두므로([DATA_PIPELINE.md](DATA_PIPELINE.md)의 "주간분석.py" 절 8번 참고) 곧바로
  표에 꽂아 보여줄 수 있음(`showHigh60Rate`와 정반대로, 일간은 기본 꺼짐·주간만 켬).
  일간 표(`showMain`)와는 형제 블록으로
...
2. **거래대금 카테고리 TOP5 추이**(`CategoryRankTrend`, 2026-06-25 도입, 2026-06-28
   ...
   `analysis.거래대금`/`analysis.등락률` 배열, 종목명+카테고리)을 받아 `themeTrend` prop으로
   내려주고(날짜 캘린더 선택과 무관 — "현재 펄스"). `Analysis.jsx`의 `rankCategoriesByDay()`가
   그날 거래대금 50종목의 카테고리별 종목 수를 세어(기타 제외 — `CategoryPieCarousel`과
   동일하게 진짜 섹터만 순위 경쟁) 상위 5개를 순위대로 뽑고, `buildCategoryRankTrend(days,
   field)`가 카테고리 데이터가 있는 날짜만 남겨(과거 미백필 날짜는 통째로 제외, 6/15~6/24
   8일치가 여기 해당 — 백필 안 하기로 확정) 표시할 날짜 수(15)보다 하루 더 가져와 **맨
   왼쪽(가장 오래된) 보이는 칸도 전날과 정확히 비교**할 수 있게 한다(그 1일치는 비교 전용
   baseline으로만 쓰고 화면엔 안 그림 — 보이는 유효 날짜가 15일이 안 되면 baseline 없이
   있는 만큼만 표시, 그래서 2026-06-28 현재는 실제로 2~3칸만 보이다가 날이 지날수록
   자연히 15칸까지 늘어남). **표 형태**(`CategoryRankTrendTable` 컴포넌트): 행=순위
   1~5위(맨 왼쪽 고정 열, `.cat-trend-rank-col`, `position: sticky`), 열=날짜(가로
   스크롤 — 캐러셀처럼 페이지 단위로 스냅하는 게 아니라 그냥 옆으로 길게 스크롤하는 일반
   표). 각 칸은 "카테고리명(종목 수)"를 보여주고, **전날 대비 순위가 오르면 빨간 글씨
   (`.cat-trend-up`, `var(--c-up)`), 내리면 파란 글씨(`.cat-trend-down`,
   `var(--c-down)`)** — 국내 증시 관행대로 오르면 빨강·내리면 파랑(주가 등락 색과 동일
   토큰 재사용). **CSS 명시도 버그로 이 색이 안 보이던 적이 있었음**(2026-06-29 발견·수정
   — 직접 확인): `.cat-trend-table td { color: var(--c-heading); ... }`가 `.cat-trend-up`/
   `.cat-trend-down`보다 셀렉터 명시도가 더 높아(`td` 엘리먼트가 더 붙음) 소스 순서와
   무관하게 항상 그 `color`를 덮어썼음 — 그 규칙엔 `font-weight`가 없어서 굵게는 정상
   보이는데 색만 항상 기본값(`--c-heading`)으로 남아 사용자가 "굵은 글씨만 보이고 색은
   안 바뀐다"고 제보해 발견함. **수정**: `.cat-trend-up`/`.cat-trend-down`/`.cat-trend-new`
   셀렉터에 `td`를 같이 넣어(`.cat-trend-table td.cat-trend-up` 등) 명시도를 그 규칙과
   같거나 높게 맞춤. **전날 TOP5에 없다가 새로 진입하면 초록 배경**(`.cat-trend-new`, 기존
   `--c-success` 톤을 옅게 재사용). `ThemeTable` 바로 아래, "주요 뉴스" 위에 위치.
   카테고리 데이터가 전혀 없으면 섹션 자체를 조용히 숨김 — `null`을 임의로 "기타"로
   채우지 않음. **같은 표를 등락률 기준으로도 바로 아래에 하나 더 보여줌**(2026-06-28
   추가, 사용자 요청 — "거래대금 추이 아래에 등락률 카테고리 TOP5 추이도") —
   `buildCategoryRankTrend(days, '거래대금')`/`buildCategoryRankTrend(days, '등락률')`를
   각각 호출해 `<CategoryRankTrendTable>`을 두 번 렌더링하는 구조로 바꿈(이전엔 `field`
   인자 없이 `거래대금`만 하드코딩). 이를 위해 `/api/getThemeTrend.js`도 projection에
   `analysis.등락률`을 추가하고 응답의 `days[].등락률`로 같이 내려줌. 두 표는 서로
   독립적으로 데이터 유무를 판단해 숨김 — 한쪽만 그 날짜에 카테고리가 없어도 다른 쪽은
   정상 표시될 수 있음.
3. **거래대금·등락률 카테고리 비중**(`CategoryPieCarousel`, 2026-06-26 도입) — 선택한 날짜의
   ...
   으로 표시**(2026-06-27 추가, `.pie-candidate`) — 기존 21개 카테고리 어디에도 잘 안 맞아
...
`IncomeStatementView`의 영업이익률·ROE·주당순이익·부채비율 4개 단일 계열 차트만
명시적으로 켬. (2026-06-28 한때 ROE·주당순이익을 한 차트로 합치면서 `metrics[i].axis:
'right'` 보조 y축을 추가했었으나, 결국 세 지표를 다시 독립된 차트로 되돌리며 쓰는 곳이
없어져 같이 제거함 — 다시 필요해지면 이 시점의 git 히스토리 참고.)
...
- **주차(W##) 클릭 시 "주간 종목 데이터"도 이 컴포넌트를 그대로 재사용**(2026-06-27 추가,
  ...
  조건부 렌더링하고, 펼친 차트 행의 `colSpan`도 열이 빠진 만큼 1씩 줄임. **`showCategory`
  prop**(기본값 `false`, 2026-06-28 추가)은 정반대로 주간 호출만 켜서(`showCategory`로
  단축 전달, JSX에서 `showCategory={true}`와 동일) "카테고리" 열을 추가함 — 일간
  `vol`/`rate`에는 이 필드가 없어 기본 꺼짐. 거래대금 표는 등락률 열과 거래대금/시가총액
  열 사이, 등락률 표는 등락률 열과 종목분석 열 사이에 들어가며, 값이 없으면(그 종목이
  카테고리 매칭에 실패한 경우) `-`로 표시. `colSpan`은 `showHigh60Rate`·`showCategory`
  둘 다 독립적으로 1씩 더해 계산(`7 + (showHigh60Rate?1:0) + (showCategory?1:0)` 형태 —
  거래대금 표 기본 7, 등락률 표 기본 5에서 시작).
...
- **등락률 컬럼**: 순위 | 종목명 | 현재가 | 등락률 | 60일 신고가대비 | 종목분석(2026-06-28 추가)
  ...
  열이 통째로 빠지고, 대신 `showCategory`로 "카테고리" 열이 추가돼 거래대금 표는 등락률·
  거래대금/시가총액 사이, 등락률 표는 등락률·종목분석 사이에 들어감, 2026-06-28 추가)
