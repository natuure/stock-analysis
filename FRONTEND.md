...
3. **거래대금·등락률 카테고리 비중**(`CategoryPieCarousel`, 2026-06-26 도입) — 선택한 날짜의
   ...
   독립적으로 관리. **두 차트는 `.pie-carousel`(CSS `scroll-snap`)로 한 화면에 하나씩 배치돼
...
개수 표시로 변경 요청). `members`(그 카테고리에 속한 종목명 배열)도 슬라이스마다 갖고
있어, 범례의 카테고리 이름을 클릭하면 `PieChart` 내부 `expanded` state로 그 슬라이스의
`members`를 펼쳐서 보여줌(`Tables.jsx`의 행 클릭 → 인라인 차트 펼침과 같은 아코디언 패턴,
도넛마다 독립적인 펼침 상태).
