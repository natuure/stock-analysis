...
- **연도별 추이 차트**(2026-06-25 추가, 사용자 요청): 손익계산서·재무상태표 둘 다 단일 최근값
  ...
  주당순이익, 부채비율 4개 라인 차트는 표식마다 값을 숫자로 같이 보여줌**(`TrendChart`의
  `showValues` prop, 2026-06-27 추가, 사용자 요청 — 자산 구성비 등 나머지 라인 차트는 적용
  안 함, 아래 "TrendChart.jsx" 절 참고). **분기 EPS를 보여주려고 종목분석.py의 `fetch_quarters()`에
...
3. **거래대금·등락률 카테고리 비중**(`CategoryPieCarousel`, 2026-06-26 도입) — 선택한 날짜의
   ...
   으로 표시**(2026-06-27 추가, `.pie-candidate`) — 기존 14개 카테고리 어디에도 잘 안 맞아
...
`StockAnalysis.jsx` 안에 있었는데, "테마 카테고리 추이"에도 같은 모양(기간×지표)이 필요해져서
별도 파일로 분리하고 두 곳에서 import. 종목/날짜 등 특정 도메인 로직이 전혀 없는 범용
컴포넌트라 추출에 따른 동작 변화 없음. 팔레트 `TREND_PALETTE`도 같이 옮기며 4색 → 6색으로
확장(`MAX_THEMES=6`/카테고리 상위 6개와 정합) — 기존 재무 추이 차트는 여전히 4계열까지만
쓰므로 영향 없음. **`showValues`(불 옵션, `type="line"`에만 적용, 2026-06-27 추가)**를 켜면
각 표식(circle) 바로 위에 `valueFormatter(value)`(기본값 `v => v.toFixed(1)`, 호출 쪽이
`fmtPct`/`fmtWon` 등으로 보통 오버라이드)로 포맷한 값을 작은 텍스트(`.trend-point-label`,
9유닛)로 같이 그림. **`showValues`일 때 내부 좌표계 높이(`H`)도 130→240, 위쪽 여백(`PAD_T`)도
...
줄임). 기본값은 꺼짐이라 기존 차트(자산 구성비·테마 카테고리 추이 등)는 영향 없고,
