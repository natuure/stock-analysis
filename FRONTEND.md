...
- **연도별 추이 차트**(2026-06-25 추가, 사용자 요청): 손익계산서·재무상태표 둘 다 단일 최근값
  ...
  손익계산서는 ① 매출액·영업이익·당기순이익 묶음 막대(억원) ② 영업이익률 꺾은선(%, 1계열)
  ③ ROE·주당순이익 꺾은선(%·원, 2계열) 3개, 재무상태표는 ① CAPEX·재고자산·매출채권·
  ...
  싶다는 사용자 요청으로 영업이익률만 단독 차트로 분리하고 ROE·주당순이익을 합쳤다
  (2026-06-28)** — `ROE = 당기순이익 ÷ 지배기업소유주지분`(분기는 지배기업소유주지분이
  없어 자본총계로 대체)이고, ROE(%, 10~30대)와 주당순이익(원, 만원대)은 값의 크기 단위가
  완전히 달라 같은 y축에 그리면 ROE가 0 근처에 눌려 안 보이므로 `TrendChart`에 보조(우측)
  y축 지원을 추가해 분리함(아래 "TrendChart.jsx" 절 참고). 값이 둘 이상인 계열은 캔들차트의
  이동평균선 범례(`.candle-legend`)와 같은 패턴의 `.trend-legend`로 색상-라벨을 표시(보조축
  계열은 라벨 뒤에 "(우축)" 표시). **영업이익률, ROE·주당순이익, 부채비율 3개 라인 차트 +
