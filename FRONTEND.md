...
- **연도별 추이 차트**(2026-06-25 추가, 사용자 요청): 손익계산서·재무상태표 둘 다 단일 최근값
  ...
  ③ ROE·주당순이익 묶음 막대(%·원, 2계열) 3개, 재무상태표는 ① CAPEX·재고자산·매출채권·
  현금 비중 꺾은선(%, 4계열) ② 부채비율 꺾은선(%) ③ 선수금·자본금·자본잉여금·이익잉여금 묶음
  막대(억원, 4계열) 3개를 그림. **②·③은 처음엔 영업이익률·ROE 한 차트(2026-06-27 ROE
  추가) + 주당순이익 별도 차트로 나뉘어 있었으나, ROE와 주당순이익을 한 차트에서 같이 보고
  싶다는 사용자 요청으로 영업이익률만 단독 차트로 분리하고 ROE·주당순이익을 합쳤다(2026-06-28),
  바로 이어서 위 매출액·영업이익·당기순이익 막대 차트처럼 막대 형태로 보여달라는 요청으로
  ②·③ 둘 다 꺾은선에서 막대로 바꿈(같은 날)** — `ROE = 당기순이익 ÷ 지배기업소유주지분`
  (분기는 지배기업소유주지분이 없어 자본총계로 대체)이고, ROE(%, 10~30대)와 주당순이익(원,
  만원대)은 값의 크기 단위가 완전히 달라 같은 y축에 그리면 ROE가 0 근처에 눌려 안 보이므로
  `TrendChart`에 보조(우측) y축 지원을 추가해 분리함(막대 차트도 좌/우축 계열별로 막대
  높이를 따로 계산, 아래 "TrendChart.jsx" 절 참고). 값이 둘 이상인 계열은 캔들차트의
  이동평균선 범례(`.candle-legend`)와 같은 패턴의 `.trend-legend`로 색상-라벨을 표시(보조축
  계열은 라벨 뒤에 "(우축)" 표시). **영업이익률, ROE·주당순이익, 부채비율(꺾은선 유지) +
  매출액·영업이익·당기순이익 막대 차트는 표식/막대마다
...
`IncomeStatementView`의 영업이익률·ROE·주당순이익·부채비율 4개 계열(3개 차트 블록)만
명시적으로 켬. **`metrics[i].axis: 'right'`(2026-06-28 추가)**를 주면 그 계열만 보조(우측)
y축 스케일로 그림 — 단위가 다른 두 계열(예: ROE %, 주당순이익 원)을 한 차트에 합치면 값의
크기 차이 때문에 한쪽이 0 근처에 눌려 안 보이는 문제를 해결. 좌/우 축의 min/max/range를
각각 따로 계산해 `axis`별로 다른 `y(value)` 변환 함수를 쓰고, `metrics[i].valueFormatter`로
계열별 포맷터도 따로 지정 가능(차트 공통 `valueFormatter` prop은 계열에 지정이 없을 때만
폴백). 0 기준선(`zeroY`)은 좌측 축(없으면 우측 축) 기준 하나만 그림 — 두 축을 동시에
표시하는 눈금/라벨은 없음(기존에도 y축 눈금 자체가 없었음). 보조축 계열은 범례에 "(우축)"을
덧붙여 구분. `axis`를 안 주면 기존과 동일하게 전부 좌측 축 하나로 계산되므로 다른 차트
(매출액·영업이익·당기순이익 막대 등)는 영향 없음.
