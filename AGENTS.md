...
기업개요·재무상태표·손익계산서·현금흐름표 4개 버튼 중 기업개요만 실제로 동작(PER/PBR/ROE/
EV·EBITDA + 목표 PER·PBR·EV·EBITDA·WACC를 슬라이더로 조절하는 적정주가 4종). `python
종목분석.py 종목명`을 실행하면 MongoDB `company_analysis` 컬렉션에 저장되고, 탭 상단 검색창에
종목명을 입력(자동완성 지원)하면 바로 조회된다(2026-06-25, 종목 검색 자동 연동).
재무상태표·손익계산서는 레이아웃만(값 `-`), 현금흐름표는 여전히 내용 미정 placeholder.
...
- **`종목분석.py`가 "종목 분석" 탭을 채워주는 건가?** → 2026-06-25부터 그렇다(자동 연동 완료).
  ...
  버튼이 `종목분석.py` 출력 구조(DART 재무제표 + KIS 현재가)를 읽어 PER/PBR/ROE/EV·EBITDA와
  적정주가 슬라이더 4종(PER법·PBR법·EV/EBITDA법·DCF법)을 계산한다. `python 종목분석.py 종목명`을
  실행하면 결과가 MongoDB `company_analysis` 컬렉션에 저장되고, 탭 상단 검색창(자동완성 지원)에
  종목명을 입력하면 `/api/getCompanyOverview`가 바로 조회해 표시 — 분석해둔 종목이 아니면
  "분석된 종목이 아닙니다" 안내가 뜬다. 재무상태표·손익계산서는 레이아웃만(값 `-`), 현금흐름표는
  내용 미정 placeholder ([DATA_PIPELINE.md](DATA_PIPELINE.md), [FRONTEND.md](FRONTEND.md)).
