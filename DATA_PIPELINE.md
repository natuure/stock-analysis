...
### 종목분석.py (2026-06-22 도입)
```bash
python 종목분석.py "종목명/현재가/시가총액/발행주식수/유통주식수"
# 예) python 종목분석.py "인지컨트롤스/6680/1056억/15809197/12237922"
```
- `/종목분석` 스킬(`.claude/skills/종목분석/SKILL.md`)의 0~2단계(입력 파싱 → DART·네이버
  데이터 수집 → 밸류에이션 계산)만 자동화한 스크립트. 서술형 본문(사업 개요, 임원/최대주주
  상세, SWOT·최종 투자의견)과 `.docx` 리포트 생성(3~4단계)은 이 결과 JSON을 보고 Claude
  Code가 직접 작성한다 — 뉴스분석.py → "분석해줘" → 저장분석.py와 같은 "스크립트는 원자료만,
  서술은 Claude Code가" 분리 구조를 그대로 따름. 메인 화면의 "종목 분석" 탭(아직 준비 중)과는
  무관한 별도 기능.
...
- **corp_code 매핑**: DART API는 종목명이 아니라 8자리 `corp_code`로 조회해야 해서, 최초
  ...
  이름이 없으면 부분일치로 폴백).
- **재무 데이터**: `fnlttSinglAcntAll.json`(단일회사 전체 재무제표)로 최근 3개 사업연도(연결,
  `CFS`, 없으면 별도 `OFS`로 폴백) + 최근 분기·반기·사업보고서 5개(시간순, 오늘 날짜부터
  거슬러 올라가며 실제 제출된 것만 채택)를 가져옴.
  ...
  - **TTM(최근 12개월) 순이익**은 분기·반기보고서 손익 계정이 연초부터의 **누적치**라서, 최근
    보고서 4개를 단순히 더하면 사업보고서(12개월)+3분기(9개월 누적)+반기(6개월 누적)+분기
    (3개월)가 겹쳐서 더해져 크게 부풀려진다 — `최근 사업보고서 순이익 - 작년 동기 누적치 +
    올해 동기 누적치`로 표준 롤포워드 계산함(`compute_ttm_net_income()`).
  - **차입금**(EV 계산용)은 재무상태표(`sj_div='BS'`)로 한정해서 합산함 — 현금흐름표에도
    `차입금의 차입`/`차입금의 상환` 같은 유사한 이름의 **흐름(flow)** 항목이 있어, 필터 없이
    부분일치만 하면 잔액(stock)과 흐름이 섞여 실제 자산총계보다 큰 말도 안 되는 차입금이
    나왔던 버그가 있었음(직접 검증 후 수정).
  - **EPS**는 직접 계산(`당기순이익/발행주식수`)보다 DART가 보고하는 `기본 주당이익`을
    우선 사용(가중평균주식수 반영이라 더 정확) — 둘 다 있으면 DART 값을 씀.
- **공시 목록**: `list.json`(공시검색)으로 최근 1년 공시 제목·날짜만 가져옴(상세 본문 파싱은
  안 함).
- **네이버 뉴스/웹 검색**: 종목명 기준 4개 쿼리(`{name} 실적 수주 계약`, `{name} 전망 이슈
  2026`, `{name} PER PBR ROE 밸류에이션 배당`, `{name} 임원 최대주주 주식보유 경력`).
- 결과: `종목분석결과/{종목명}_{YYYYMMDD}.json` 저장 (`.gitignore` 처리됨)
- **이 스크립트가 수집하지 않는 것**: 사업의 개요·매출/수주 현황 같은 서술형 사업보고서
  본문, 임원현황, 최대주주현황 — DART 엔드포인트 확신도가 낮아 의도적으로 제외함. 리포트
  작성 시 dart-mcp나 웹 검색으로 보완 필요(스크립트 실행 후 출력되는 안내 메시지에도 명시됨).
