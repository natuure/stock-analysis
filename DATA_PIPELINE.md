...
- **재무 데이터**: `fnlttSinglAcntAll.json`(단일회사 전체 재무제표)를 다음 순서로 가져옴
  ...
  - **기본 주당이익**은 DART가 직접 보고하는 값(가중평균주식수 반영이라 `당기순이익/발행주식수`
    ...
    Code가 필요시 계산. 계정명도 공백·`(손실)` 접미사 유무로 4가지 표기(`기본 주당이익`/
    `기본주당이익`/`기본 주당이익(손실)`/`기본주당이익(손실)`)가 다 쓰여서 모두 후보에 포함
    (마지막 표기는 일지테크에서 직접 확인, 2026-06-25). 그래도 못 찾으면(공시 자체가 없는
    회사도 있음, 직접 확인) `null`로 남음, 정상 동작.
