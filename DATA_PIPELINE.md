...
- **UN 모드의 함정**: NXT에서 전혀 거래되지 않는 종목(우선주 등 일부)은 `UN`이 빈
  `output2`를 반환함(직접 확인). 이 경우 `J`(KRX 단독)로 재조회한다 — NXT 거래가
  없으니 KRX 단독이 곧 진짜 통합값과 같다.
...
- 검증 결과(2026-06-19 기준, 180개 후보): 178개 KIS 성공·2개 FDR 폴백. SK하이닉스
  거래대금이 FDR 단독 20.8조원 → KIS 통합 34.7조원으로, 거래대금·등락률 상위 50
  종목 구성 자체가 FDR 단독과 달라짐(NXT 비중이 큰 종목이 순위 상승).
...
- 이미 MongoDB에 저장된 과거 5개 날짜(06-15~19)는 이번 변경 이전 FDR 단독 값 그대로다
  (앞으로 실행되는 분량부터 적용, 과거 날짜 재계산은 따로 요청 시 진행).
