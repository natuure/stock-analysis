...
// → 단위가 다르므로 거래대금/시가총액처럼 둘을 나누는 계산은 Cards.jsx에서 *100 보정 필요
```
...
> 단, 이미 MongoDB에 저장된 과거 날짜의 `stock_data` 문서는 그대로 원 단위로 남아있으므로
> `뉴스분석.py`가 그 날짜로 다시 실행되기 전까진 과거 날짜를 열면 옛 단위(큰 숫자)로 보인다.
